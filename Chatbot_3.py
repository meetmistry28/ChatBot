...
class DocChatBot:
    ...
    def _preprocess_text(self, text):
        text = self._normalize_abbreviations(text) if hasattr(self, '_normalize_abbreviations') else text
    ...
    def detect_topic(self, query):
        ...
        for topic in self.known_topics:
            if topic in query:
                return re.sub(r'\s+', '_', topic)  # e.g. "deep learning" -> "deep_learning"
        return "misc"
    ...
    def extract_main_keyword(question):
        ...
        # Prefer proper nouns (NNP), then nouns (NN)
        proper_nouns = [word for word, tag in tagged if tag == "NNP"]
        common_nouns = [word for word, tag in tagged if tag == "NN"]

        if proper_nouns:
            main_word = proper_nouns[0]
        elif common_nouns:
            main_word = common_nouns[0]
        else:
            # fallback: use first non-stopword token
            main_word = tokens[0]

        return re.sub(r'[^a-zA-Z0-9_\-]', '_', main_word.lower())



    
    def get_topic_filename(self, query):
        main_keyword = self.extract_main_keyword(query)
        folder = "topics"
        os.makedirs(folder, exist_ok=True)
        return os.path.join(folder, f"{main_keyword}.json")
