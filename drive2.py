    ...
    for url in urls:
        try:
            ...
            for p in paragraphs:
                ...
                if (
                    raw_text
                    and raw_text not in seen_texts
                    and is_meaningful_text(raw_text, query_keywords)
                ):
...
class DocChatBot:
    ...
    def get_response(self, query):
        ...
        if question_data:
            # If fewer than 3 answers, fetch a new one from web
            if len(question_data["answer"]) < 10:
                ...
                for ans in new_answers:
                    ...
                    if cleaned not in question_data["answer"]:
                        ...
                        return cleaned
                # All new answers were duplicates — fallback to random
            return random.choice(question_data["answer"])

        # If question not found at all in topic file
